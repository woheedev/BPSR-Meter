...
async function saveWindowSize(
    ...
    try {
        ...
        settings.windowSizes = {
            ...settings.windowSizes,
            [windowType]: lastWindowSizes[windowType],
        };
        ...
        exec(`netstat -ano | findstr :${port}`, (error, stdout) => {
            ...
            if (pid) {
                exec(`taskkill /PID ${pid} /F`, (killError) => {
                    if (!killError)
                        logToFile(`Process ${pid} killed successfully`);
                    resolve();
                });
            } else {
                resolve();
...
function setupIpcHandlers() {
    ...
    ipcMain.on("open-group-window", () =>
        createOrFocusWindow("group").catch((err) =>
            logToFile(`Error opening group window: ${err}`),
        ),
    );
    ipcMain.on("open-history-window", () =>
        createOrFocusWindow("history").catch((err) =>
            logToFile(`Error opening history window: ${err}`),
        ),
    );
    ipcMain.on("open-device-window", () =>
        createOrFocusWindow("device").catch((err) =>
            logToFile(`Error opening device window: ${err}`),
        ),
    );
    ipcMain.on("open-settings-window", () =>
        createOrFocusWindow("settings").catch((err) =>
            logToFile(`Error opening settings window: ${err}`),
        ),
    );
            ...
            try {
                ...
                if (settings.hasOwnProperty("lockKeybind")) {
                    ...
                    const currentKeybinds = {
                        lockKeybind:
                            settings.lockKeybind || "CommandOrControl+L",
                        monstersKeybind:
                            currentSettings.monstersKeybind ||
                            "CommandOrControl+M",
                        groupKeybind:
                            currentSettings.groupKeybind ||
                            "CommandOrControl+G",
                        settingsKeybind:
                            currentSettings.settingsKeybind ||
                            "CommandOrControl+S",
                        deviceKeybind:
                            currentSettings.deviceKeybind ||
                            "CommandOrControl+D",
                        historyKeybind:
                            currentSettings.historyKeybind ||
                            "CommandOrControl+H",
                    };

                    // Register lock keybind
                    const ret = globalShortcut.register(
                        currentKeybinds.lockKeybind,
                        () => {
                            isLocked = !isLocked;
                    ...
                    const toggleWindow = (windowType: WindowType) => {
                        ...
                        if (window && !window.isDestroyed()) {
                            ...
                            createOrFocusWindow(windowType).catch((err) =>
                                logToFile(
                                    `Error opening ${windowType} window: ${err}`,
                                ),
                            );
                        }
                    };
                    ...
                    globalShortcut.register(
                        currentKeybinds.monstersKeybind,
                        () => toggleWindow("monsters"),
                    );
                    globalShortcut.register(currentKeybinds.groupKeybind, () =>
                        toggleWindow("group"),
                    );
                    globalShortcut.register(
                        currentKeybinds.settingsKeybind,
                        () => toggleWindow("settings"),
                    );
                    globalShortcut.register(currentKeybinds.deviceKeybind, () =>
                        toggleWindow("device"),
                    );
                    globalShortcut.register(
                        currentKeybinds.historyKeybind,
                        () => toggleWindow("history"),
                    );
                }

                if (
                    settings.hasOwnProperty("monstersKeybind") ||
                    settings.hasOwnProperty("groupKeybind") ||
                    settings.hasOwnProperty("settingsKeybind") ||
                    settings.hasOwnProperty("deviceKeybind") ||
                    ...
                    const currentKeybinds = {
                        lockKeybind:
                            currentSettings.lockKeybind || "CommandOrControl+L",
                        monstersKeybind:
                            currentSettings.monstersKeybind ||
                            "CommandOrControl+M",
                        groupKeybind:
    ...
    ipcMain.on(
        "set-window-position",
        (event: IpcMainEvent, windowType: WindowType, x: number, y: number) => {
            const senderWindow = BrowserWindow.fromWebContents(event.sender);
            if (!senderWindow) return;

            const bounds = senderWindow.getBounds();
            const windowWidth = windowType
                ? lastWindowSizes[windowType].width
                : bounds.width;
            const windowHeight = windowType
                ? lastWindowSizes[windowType].height
                : bounds.height;
...
async function createMainWindow(): Promise<void> {
    ...
    windows.main = new BrowserWindow(
        createWindowConfig("main", lastWindowSizes),
    );
...
app.whenReady().then(() => {
    ...
    try {
        if (fs.existsSync(settingsPath)) {
            ...
            if (settings.monstersKeybind)
                savedMonstersKeybind = settings.monstersKeybind;
            if (settings.groupKeybind)
                savedGroupKeybind = settings.groupKeybind;
            if (settings.settingsKeybind)
                savedSettingsKeybind = settings.settingsKeybind;
            if (settings.deviceKeybind)
                savedDeviceKeybind = settings.deviceKeybind;
            if (settings.historyKeybind)
                savedHistoryKeybind = settings.historyKeybind;
        }
    } catch (error) {
    ...
    const monstersRet = globalShortcut.register(savedMonstersKeybind, () =>
        toggleWindow("monsters"),
    );
    if (!monstersRet) {
        logToFile(
            `Failed to register monsters keybind: ${savedMonstersKeybind}`,
        );
    }

    const groupRet = globalShortcut.register(savedGroupKeybind, () =>
        toggleWindow("group"),
    );
    if (!groupRet) {
    ...
    const settingsRet = globalShortcut.register(savedSettingsKeybind, () =>
        toggleWindow("settings"),
    );
    if (!settingsRet) {
        logToFile(
            `Failed to register settings keybind: ${savedSettingsKeybind}`,
        );
    }

    const deviceRet = globalShortcut.register(savedDeviceKeybind, () =>
        toggleWindow("device"),
    );
    if (!deviceRet) {
    ...
    const historyRet = globalShortcut.register(savedHistoryKeybind, () =>
        toggleWindow("history"),
    );
    if (!historyRet) {
