...
export interface DungeonTargetProgress {
  targetId: number;
  targetProgress: number;
  awardState: number;
}

function createBaseDungeonTargetProgress(): DungeonTargetProgress {
  return { targetId: 0, targetProgress: 0, awardState: 0 };
}

export const DungeonTargetProgress: MessageFns<DungeonTargetProgress> = {
  encode(message: DungeonTargetProgress, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): DungeonTargetProgress {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseDungeonTargetProgress();
  ...
  fromJSON(object: any): DungeonTargetProgress {
    return {
  ...
  toJSON(message: DungeonTargetProgress): unknown {
    const obj: any = {};
    if (message.targetId !== 0) {
  ...
  create<I extends Exact<DeepPartial<DungeonTargetProgress>, I>>(base?: I): DungeonTargetProgress {
    return DungeonTargetProgress.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<DungeonTargetProgress>, I>>(object: I): DungeonTargetProgress {
    const message = createBaseDungeonTargetProgress();
